...
APP_VERSION="1.6.0"; START=date(2026,9,1); DEFAULT_BALANCE=20.71; DEFAULT_DEPOSIT=10.0; DEFAULT_NEXT=date(2026,9,11)
...
def style_chart(fig,h=390):
    fig.update_layout(height=h,margin=dict(l=8,r=8,t=32,b=8),paper_bgcolor="rgba(0,0,0,0)",plot_bgcolor=NAVY,font_color=WHITE,xaxis=dict(gridcolor="#2B5AA0",tickfont=dict(color=WHITE)),yaxis=dict(gridcolor="#2B5AA0",tickfont=dict(color=WHITE)),legend=dict(orientation="h",y=1.07),hovermode="x unified")

st.set_page_config(page_title="High-Yield Savings Project",page_icon="💰",layout="wide",initial_sidebar_state="collapsed")
st.markdown('''<style>
:root{--blue:#087CF2;--deep:#053B98;--navy:#061A45;--red:#E51B23;--orange:#FF6A00;--yellow:#FFE600;--white:#fff}.stApp{background:linear-gradient(180deg,#087CF2 0%,#075dcc 20%,#061A45 72%,#04102d 100%);color:white}.block-container{max-width:1380px;padding:.35rem .7rem 3rem}header,footer{visibility:hidden}.brand{font-family:Impact,Arial Black,sans-serif;font-size:clamp(2rem,5vw,4.1rem);line-height:.9;font-style:italic;color:white;text-shadow:3px 3px 0 #E51B23;margin:.15rem 0 .55rem}.brand span{color:#FFE600}.hero{border:4px solid #FFE600;border-radius:20px;padding:5px;background:#061A45;box-shadow:0 8px 0 #E51B23}.hero img{width:100%;display:block;border-radius:13px}.banner{background:linear-gradient(90deg,#E51B23,#FF6A00);color:white;border:3px solid #FFE600;border-radius:14px;padding:12px 14px;margin:12px 0;font-weight:1000}.section-title{font-size:1.25rem;font-weight:1000;color:white;margin:.8rem 0 .35rem}.goalbar{height:20px;background:#061A45;border:2px solid white;border-radius:999px;overflow:hidden}.goalfill{height:100%;background:linear-gradient(90deg,#FFE600,#FF6A00,#E51B23)}.goalcopy{font-weight:900;color:white;margin-top:5px}[data-testid="stVerticalBlockBorderWrapper"]{background:rgba(6,26,69,.93);border:2px solid #FF6A00!important;border-radius:16px!important}.stButton button{width:100%;min-height:72px;background:linear-gradient(145deg,#087CF2,#053B98)!important;color:white!important;border:3px solid #FFE600!important;border-radius:15px!important;font-weight:1000!important;font-size:1rem!important;white-space:pre-line!important;line-height:1.25!important;box-shadow:0 5px 0 #E51B23!important}.stButton button:hover,.stButton button:focus{background:linear-gradient(145deg,#E51B23,#FF6A00)!important;color:white!important;border-color:white!important}.stButton button p{color:white!important}.action-panel{background:#061A45;border:3px solid #FF6A00;border-left:8px solid #FFE600;border-radius:15px;padding:15px;margin:10px 0;color:white}.action-panel b{color:#FFE600}.big{font-family:Impact,Arial Black;font-size:clamp(2.5rem,7vw,4.5rem);color:white;text-shadow:3px 3px 0 #E51B23;line-height:1}.insight{background:linear-gradient(90deg,#053B98,#087CF2);border:2px solid #FFE600;border-radius:13px;padding:12px;margin:8px 0;color:white;font-weight:900}[data-testid="stPlotlyChart"]{border:2px solid #FF6A00;border-radius:14px;overflow:hidden;background:#061A45}[data-baseweb="select"]>div,[data-baseweb="input"]>div,input{color:#061A45!important;background:white!important}input{font-size:18px!important;font-weight:900!important}.stNumberInput label,.stDateInput label,.stSelectbox label,.stRadio label{color:white!important;font-weight:900!important}.deposit-wrap{background:linear-gradient(135deg,#E51B23,#FF6A00);border:3px solid #FFE600;border-radius:18px;padding:12px 14px 5px;margin-bottom:8px;box-shadow:0 6px 0 #053B98}.deposit-title{font-family:Impact,Arial Black;font-size:1.35rem;color:white;letter-spacing:.4px}.deposit-help{font-size:.86rem;font-weight:800;color:white}.stAlert p{color:#061A45!important}.stAlert{border:2px solid #FFE600!important}
@media(max-width:700px){.block-container{padding:.2rem .38rem 2rem}.brand{font-size:2.25rem}.hero{border-width:3px;box-shadow:0 5px 0 #E51B23}.banner{font-size:.92rem}.section-title{font-size:1.08rem}[data-testid="column"]{min-width:100%!important;width:100%!important;flex:1 1 100%!important}.stButton button{min-height:66px;font-size:.95rem;margin-bottom:.25rem}.action-panel{font-size:.94rem}.big{font-size:3rem}.deposit-wrap{padding:10px 11px 3px}.deposit-title{font-size:1.2rem}}
...
st.markdown("<div class='banner'>⚡ TYPE YOUR SAVINGS NUMBER. No touchy deposit slider — enter exactly what you want to save and the entire dashboard recalculates.</div>",unsafe_allow_html=True)
with st.container(border=True):
    st.markdown("<div class='section-title'>🎮 CONTROL YOUR PLAN</div>",unsafe_allow_html=True)
    c1,c2=st.columns(2)
    with c1:
        st.markdown("<div class='deposit-wrap'><div class='deposit-title'>💵 YOUR PAYCHECK DEPOSIT</div><div class='deposit-help'>Type the exact amount you want automatically moved to savings each paycheck.</div></div>",unsafe_allow_html=True)
        amount=st.number_input("Deposit amount ($)",min_value=0.0,max_value=5000.0,value=DEFAULT_DEPOSIT,step=1.0,format="%.2f",key="deposit_amount")
        q1,q2,q3,q4=st.columns(4)
        with q1:
            if st.button("$10",key="q10"):st.session_state.deposit_amount=10.0;st.rerun()
        with q2:
            if st.button("$25",key="q25"):st.session_state.deposit_amount=25.0;st.rerun()
        with q3:
            if st.button("$50",key="q50"):st.session_state.deposit_amount=50.0;st.rerun()
        with q4:
            if st.button("$100",key="q100"):st.session_state.deposit_amount=100.0;st.rerun()
    with c2:
...
if st.session_state.focus=="balance":st.markdown(f"<div class='action-panel'><b>BALANCE ACTION</b><div class='big'>{money(balance)}</div>Edit the balance above whenever the real account changes.</div>",unsafe_allow_html=True)
    ...
    st.markdown("<div class='action-panel'><b>CLOSE-THE-GAP CALCULATOR</b><br>Type the number of months you want to give yourself.</div>",unsafe_allow_html=True)
    target_months=st.number_input("Months to reach $1,000",min_value=1,max_value=120,value=12,step=1)
    target_date=START+timedelta(days=round(target_months*30.4375));candidates=scenarios(cfg,range(5,1001,5));possible=candidates[candidates["Goal Date"]<=target_date];need=None if possible.empty else int(possible.iloc[0]["Deposit"])
    ...
    st.markdown(f"<div class='action-panel'><b>NEXT DIVIDEND BREAKDOWN</b><div class='big'>≈ {money(div)}</div>Deposits before payout: {money(detail.deposit.sum())} on {dates}. Estimated pre-dividend balance: {money(pre)}.</div>",unsafe_allow_html=True)
    x=detail.copy();x["cumulative"]=x.accrual.cumsum();fig=go.Figure(go.Scatter(x=x.date,y=x.cumulative,mode="lines+markers",line=dict(color=ORANGE,width=5),marker=dict(color=YELLOW,size=6),fill="tozeroy",fillcolor="rgba(255,106,0,.16)"));style_chart(fig,320);fig.update_yaxes(tickprefix="$",tickformat=".2f");st.plotly_chart(fig,use_container_width=True,config={"displayModeBar":False})
...
st.markdown("<div class='section-title'>🚀 QUICK WHAT-IF BUTTONS</div>",unsafe_allow_html=True)
cols=st.columns(6)
...
st.markdown(f"<div class='goalcopy' style='text-align:center;margin-top:18px'>HIGH-YIELD SAVINGS PROJECT v{APP_VERSION} • TYPE IT • TEST IT • GROW IT</div>",unsafe_allow_html=True)